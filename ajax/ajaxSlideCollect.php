...
$selectCollect = "SELECT * FROM `collection` WHERE `u_uid` = '$userID' ORDER BY `cn_cid` DESC";
$queryCollect = mysqli_query($link, $selectCollect);
while ($resultCollect = mysqli_fetch_array($queryCollect)) {
	$caseID = $resultCollect['c_cid'];
	$selectGet = "SELECT * FROM `cases` WHERE `c_cid` = '$caseID' ORDER BY `c_cid` DESC";
